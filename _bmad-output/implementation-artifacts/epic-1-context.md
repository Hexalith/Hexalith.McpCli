...
- Invalid Operation declarations are excluded while valid ones remain discoverable; an invalid Module marker or serializer provider excludes that Module. Diagnostics use only the defined category and severity set, identify the declaration and cause, and are logged once on stderr. Scan assemblies and types ordinally so the first duplicate survives. A marked empty Module stays visible with a warning; unmarked assemblies are silent. An empty Catalog fails Catalog-dependent actions; strict mode fails on any diagnostic. Description lint is separate from Catalog diagnostics and does not exclude an Operation.
...
- Cache one read-only Payload serializer-options instance per Module. Preserve provider converters in order while fixing canonical non-converter settings and the metadata resolver. Export Schema once per Operation; normalize only a Command's root to a non-null object. Reject unsupported free-form, polymorphic, extension-data, or converter-opaque Schema members. Resolve property-reference names as exact top-level CLR names through effective serialization metadata, cache serialized names and JSON Pointers, and reject missing, ignored, ambiguous, opaque, or conflicting property roles. Compile aggregate accessors during Catalog construction.
