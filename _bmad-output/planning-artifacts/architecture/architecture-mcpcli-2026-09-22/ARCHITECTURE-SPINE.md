...
sources:
  - _bmad-output/planning-artifacts/architecture/architecture-mcpcli-2026-09-22/VALIDATION-REPORT-2026-09-23-1241.md
  - _bmad-output/planning-artifacts/architecture/architecture-mcpcli-2026-09-22/VALIDATION-REPORT-2026-09-23.md
...
- **Rule:** the Schema is `JsonSchemaExporter.GetJsonSchemaAsNode(module.ModulePayloadOptions, type)` with one `TransformSchemaNode` in `Hexalith.McpCli.Core.Schema` that adds `description` from `[Description]`, sets `readOnly: true` and removes from `required` each envelope-filled property (AD-9), types identifiers per AD-8, removes get-only members that have neither a setter nor a constructor parameter (such as `ICommandContract.AggregateId`), and sets `additionalProperties: false`. `EnvelopeFilledProperties` is exactly the set named by `tenantProperty`, `correlationProperty`, `idempotencyKeyProperty`, and `actorProperty`; there is no name-based inference, so a `TenantId` not named by `tenantProperty` is ordinary Payload. The Command Schema root is a non-null JSON object even when the exporter emits an object-or-null root; after export, normalize only the top-level `type` to `object` and preserve nullable nested members. Core rejects a null or non-object Command Payload at `/` before envelope filling or accessor use. A Query root follows its declared contract Schema and the pinned Gateway's optional Payload rule. Validation is `JsonSchema.Net` with `OutputFormat.List`; each violation reports `InstanceLocation` (a JSON Pointer) as its path. The Schema is derived once per Operation at Catalog build and stored on the descriptor. Pre-fill validation retains the original raw Payload for AD-9 ownership checks and validates a copy with only the AD-3 mapped envelope-owned members removed against this stored Schema; ordinary members, unknown names, and root constraints are still checked. No contract deserialization occurs in this pass. After ownership checks and filling, validate the complete rebuilt Payload against the unmodified stored Schema, including envelope-member types and patterns. `readOnly` is descriptive metadata, never the mechanism that skips raw-field validation. `describe --lint` recursively inspects serialized members inside nested objects and collections; each property-level finding carries its escaped RFC 6901 pointer in Module-serialized casing and stays out of Catalog diagnostics.
...
- **Rule:** construct the five Generic Tools once with `McpServerTool.Create(delegate, McpServerToolCreateOptions { SerializerOptions = McpCliJson.Result, UseStructuredContent = true, OutputSchema })`. Each `OutputSchema` has root `type: object` and a `oneOf` of two closed object branches: the exact AD-5 success record, or `{ error: OperationError }` with required `error` and no success fields; Core owns the document contract used to derive both branches. Annotations: `ReadOnly = true` for four discovery/query tools; `ReadOnly = false, Idempotent = false, Destructive = true` for `send_command`. A failure returns `IsError = true`, `{ "error": ... }` as `structuredContent`, and the same JSON as `Content[0].Text`; no exception escapes the tool boundary. After settings resolution, register exactly the four or five permitted tools in one `ToolCollection` and use the ModelContextProtocol 2.2.0 built-in list/call dispatch. A `ListToolsFilters` filter runs after the SDK assembles `tools/list`; it projects the collection's permitted tools into the fixed sequence `list_modules`, `list_operations`, `describe_operation`, `send_command` when writable, then `run_query`, rejecting duplicates or unexpected names. The same restricted collection remains the call-dispatch source; no custom list or call handler is registered. Read-only registration omits `send_command`, and executor preflight still rejects writes. Tests assert unique names, fixed order, Read-only omission, dispatch, and success/error `structuredContent` conformance to each session-advertised schema under both a pre-2026-07-28 MCP version and the 2026-07-28 revision. Tool descriptions carry FR-9's three labeled parts; a test counts exactly the five tool names, descriptions, and input schemas under 8,000 characters and measures output schemas separately. Before the server starts serving MCP requests, settings/Catalog/strict failures emit exactly one structured error on stderr, zero stdout bytes, and exit 2; once requests can be served, failures travel through MCP only. Tests cover the legacy `initialize` handshake and the 2026-07-28 handshake-free discovery path separately. The composition root applies the AD-13 MCP output-option policy before Catalog construction or transport startup. The server uses `WithStdioServerTransport()`; logging uses `ClearProviders()` plus console with `LogToStandardErrorThreshold = Trace`.
...
- **Rule:** the out-of-process tool Catalog contains only flagged production Contracts assemblies; a test-only benchmark builds a production-like Catalog from those assemblies plus one declared synthetic sample Module through `CatalogBuilder.Build`. Isolated Catalog tests cover the other Identifier Kind. McpCli maintainers own one versioned, closed conformance-vector contract and compatibility validator in a companion test artifact before upstream vector authoring; the contract fixes generic prerequisite calls, Payload and Envelope inputs, scripted Gateway responses, semantic assertion vocabulary, and supported format versions. Each v1 Module owns vectors in its repository, matched by canonical Operation Name to every decorated Operation. A vector supplies those contract-defined inputs and assertions; its Module maintainer approves it only after the shared validator passes. Each vector declares its owning Contracts package ID and exact version; the shared validator compares these with the owning immutable Contracts release artifact before maintainer approval and with the flagged restored production package before both runner lanes, so a reused Operation Name under a changed package version is stale. The generic runner validates both Modules' vectors with that same contract before execution and rejects unsupported format versions or unknown fields. The generic runner in this repository rejects missing, duplicate, or stale vectors and contains no Module branches or domain setup code. A loopback HTTP Gateway capture/replay lane drives the built `hexalith` CLI process and MCP stdio client against the same Catalog and reset scripted Gateway responses, asserting AD-5 document equality for discovery, execution, and error cases for every Operation after masking only generated message/correlation identifiers. It also compares captured Gateway method, path, routing, Envelope, Payload, and paging between Heads and against the vector's expected inputs, masking only generated message/correlation identifiers. The loopback server requires no production test hook; supplied idempotency keys remain equal and are never masked. A separate live lane runs each vector once against one Aspire topology, asserting accepted Commands, returned Query documents, and semantic effects without comparing two live calls for byte equality. `tests/Hexalith.McpCli.AppHost` composes EventStore and Tenants through published Aspire helpers; a published `Hexalith.Parties.Aspire` helper with no server package dependency is a prerequisite for Parties. The helpers locate server source in root-declared `references/` checkouts. `tests/Hexalith.McpCli.IntegrationTests` uses `AspireTopologyFixtureBase`, the standard Dapr prerequisite check, and out-of-process heads. Tenants is not Gateway-ready until its owning repository aligns audit filter handling with the effective serialized `From`, `To`, and `Category` members and migrates every exposed paged Query handler to `QueryEnvelope.Paging` (AD-20). Approved live vectors must use non-default audit filters with discriminating fixture data and assert the actual filtered records; paging vectors request a non-default page size and a subsequent cursor and assert page contents. Retained legacy Payload paging fields cannot override or supply paging for generic envelope requests. The decorated Contracts release and corresponding handler source used by the live topology must pass these semantic gates before v1 coverage is claimed. Release CI sets `aspire-test-project`, `test-platform: microsoft-testing-platform`, and `aspire-continue-on-error: false`; a bounded upstream source-build path or helper-managed source build must land before this gate is claimed as runnable. Unit tests use `FakeEventStoreGatewayClient` or NSubstitute without a network.
...
- **Rule:** `pageSize`, `offset`, `cursor` exist only as `RunQueryArguments` members and map to `SubmitQueryRequest.Paging`; `QueryResult.Paging` maps `Metadata.Paging`. Core rejects a supplied `pageSize` outside 1..200, negative `offset`, `cursor` longer than 4,096 characters, or nonblank `cursor` combined with `offset`, returning `validation_failed` at `/pageSize`, `/offset`, or `/cursor` before any Gateway call. A stricter deployment may still return `gateway_error`. A Query Payload member named `PageSize`, `Offset`, or `Cursor` is ordinary Payload and a `describe --lint` warning `payload_paging_member`; it never supplies generic paging. Exposed paged handlers must consume `QueryEnvelope.Paging`; upstream migration and live semantic evidence are mandatory for Tenants readiness (AD-16), even if legacy Payload members remain for other clients.
...
| Catalog diagnostics | `CatalogDiagnostic(TypeName, Category, Severity, Message)`; error categories `missing_description`, `missing_routing_values`, `invalid_routing_value`, `invalid_serializer_options_provider`, `duplicate_operation_name`, `invalid_example`, `invalid_identifier_type`, `tenant_is_aggregate_id`, `invalid_property_reference`, `conflicting_property_roles`, `ambiguous_aggregate_id`, `duplicate_module`, `invalid_module_declaration` (a Module marker name, description, identifier kind, or wire type convention is invalid, or the Contracts assembly's types cannot be loaded), `conflicting_operation_kinds` (a type carries both Command and Query decorations), `invalid_operation_declaration` (a decorated type is abstract, open generic, or not a class), `invalid_operation_name` (an explicit or type-derived Operation Name part is not lowercase ASCII kebab-case), `invalid_schema` (extension data, polymorphism, a free-form (`object`, `JsonElement`, `JsonDocument`, `JsonNode`) or converter-opaque member, a Command payload that is not a JSON object, or a System.Text.Json contract error); no other category is emitted; warning categories `conflicting_value`, `redundant_value`, `empty_module`; written once to stderr at first Catalog build through `ILogger`; catalog diagnostics stay on stderr and out of public `list_modules`; invocation fails only on an empty Catalog or `--strict` with any diagnostic. |
...
sequenceDiagram
  ...
  X->>X: parse and pre-fill validate copy; check raw ownership and resolve envelope values
  X->>X: fill and validate complete Payload; resolve AggregateId; validate extensions
...
| FR-22 Upstream rule | authoritative Hexalith.AI.Tools instructions, paired release preflight | AD-17 |
...
- **Payload-level paging mappings** (`pageSizeProperty` and kin on the Query attribute): outside v1; any future support requires a revision to AD-20. Tenants must first meet the envelope-paging readiness gate in AD-16.
...
## Open Questions
- **Tenants wire-readiness prerequisite.** Tenants maintainers must merge the audit-filter casing and envelope-paging migrations and supply AD-16 semantic vectors before Gateway-ready acceptance and the v1 live gate. Current source reads lowercase audit filters and Payload paging; successful status alone does not close this prerequisite.
- **FR-22 instruction-release prerequisite.** Hexalith.AI.Tools maintainers must merge the authoritative no-new-server/CLI rule; McpCli maintainers must record the included baseline revision and successful local entry-point synchronization check before paired v1 publication (AD-17). The current baseline lacks the rule.
...
- **§5.1, FR-6/7/16 (AD-3/7/9/19):** property references name CLR members and share one serialized-name mapping; conflicting ownership is excluded. Pre-fill validation omits envelope-owned members from a copy, ownership checks retain raw values, and complete Payload validation follows filling.
- **FR-20 and §8.1 (AD-16/20):** Tenants audit filters and envelope paging require upstream migration and semantic vectors before v1 readiness.
- **FR-13 and addendum §E (AD-13):** user-adopted MCP output-option policy ignores valid inherited format, accepts explicit JSON, and rejects explicit table/output options before startup.
- **FR-22 (AD-17):** paired publication is blocked until the upstream rule is merged and the referenced baseline and local entry-point synchronization are verified.
